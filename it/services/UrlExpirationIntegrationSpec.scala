...
object TestData {
  ...
  def metadata(fileReference: String = "file-reference",
...
class UrlExpirationIntegrationSpec extends UnitSpec
                                   with GuiceOneServerPerSuite with GuiceableModuleConversions
                                   with WithWireMock {
  ...
  "receiveMessage" should {
    "generate pre-signed download url with expiration period derived from application.conf Test section" in {
      Mocks.setup(IntegrationTestsApplication.mockAmazonSQS, TestData.bucketName, TestData.objectKey, TestData.outboundMessage)
      Mocks.setup(IntegrationTestsApplication.mockAmazonS3,  TestData.bucketName, TestData.objectKey, TestData.metadata(), TestData.expirationUrl)
      ...
      val notifyOnSuccessfulFileUploadMessageProcessingJob = app.injector.instanceOf[NotifyOnSuccessfulFileUploadMessageProcessingJob]

      val result: Future[Unit] = notifyOnSuccessfulFileUploadMessageProcessingJob.run()

      Await.result(result, 1.seconds)

      val loggedRequests = wireMockServer.findAll(WireMock.postRequestedFor(WireMock.urlMatching(TestData.callbackPath)))

      if (loggedRequests.isEmpty) fail(s"WireMock did not receive a notification callback for url: [${TestData.callbackUrl}].")
